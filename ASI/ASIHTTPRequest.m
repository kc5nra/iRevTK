...
- (NSMutableDictionary *)findCredentials
{
	...
	NSString *pass = [[self url] passwordLabel];
	
	// If the username and password weren't in the url
	if (!user || !pass) {
		
		// If this is a HEAD request generated by an ASINetworkQueue, we'll try to use the details from the main request
		if ([self mainRequest] && [[self mainRequest] usernameLabel] && [[self mainRequest] passwordLabel]) {
			user = [[self mainRequest] usernameLabel];
			pass = [[self mainRequest] passwordLabel];
